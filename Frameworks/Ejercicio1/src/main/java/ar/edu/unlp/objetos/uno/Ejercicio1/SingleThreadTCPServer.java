...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                beforeCommunication(clientSocket, inputLine);
                
                if(closeConnection(inputLine)) {
                	break;
                }
                handleMessage(inputLine, out);
            }
            afterCommunication(clientSocket);
        } catch (IOException e) {
        	onConnectionError(e);
        } finally {
            try {
	...
	protected boolean closeConnection (String inputLine) {
	...
	protected void closeError(IOException e) {
